  ...
  public void onTxEvent(GrpcTxEvent message, StreamObserver<GrpcAck> responseObserver) {
    ...
    if (message.getType().equals(EventType.SagaStartedEvent.name())) {
      ...
      event = TxCompensateAckEvent.builder()
          ...
          .createTime(new Date())
          .localTxId(message.getLocalTxId()).build();
      omegaCallbacks.get(message.getServiceName()).get(message.getInstanceId()).ask(CompensateAskType.Succeed);
      ...
      event = TxCompensateAckEvent.builder()
          ...
          .createTime(new Date())
          .localTxId(message.getLocalTxId()).build();
      omegaCallbacks.get(message.getServiceName()).get(message.getInstanceId()).ask(CompensateAskType.Failed);
    } else {
      ok = false;
    }
    if (event != null) {
